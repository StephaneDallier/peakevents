...
export default function Home() {
  ...
  async function handleRegister(e) {
    e.preventDefault()
    setRegError('')
    if (!regFirstName.trim() || !regLastName.trim()) { setRegError('Prenom et nom sont obligatoires'); return }
    if (regPassword.length < 6) { setRegError('Le mot de passe doit faire au moins 6 caracteres'); return }
    ...
    <div style={{ minHeight: '100vh', background: '#f9fafb', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ padding: '72px 60px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 20 }}>
          {[
            { svg: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>, title: 'Gestion des postes', desc: "Creez et gerez les postes avec les creneaux horaires et consignes." },
            { svg: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>, title: 'Benevoles', desc: "Gerez votre annuaire de benevoles et leurs competences." },
            { svg: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>, title: 'Planning intelligent', desc: "Visualisez le planning global par poste ou par benevole." },
            { svg: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>, title: 'Affectations', desc: "Affectez les benevoles aux postes et suivez les postes incomplets." }
          ].map((f, i) => (
  ...
  if (view === 'register') return (
    <div style={{ minHeight: '100vh', background: '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 36, width: '100%', maxWidth: 440, border: '1px solid #e5e7eb', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ fontSize: 22, fontWeight: 800, marginBottom: 4, color: '#111' }}>Creer un compte</div>
        <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 24 }}>Rejoignez PeakEvents en tant que benevole</div>

        {regSuccess ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>✅</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#111', marginBottom: 8 }}>Compte cree !</div>
          ...
          <>
            ...
            <form onSubmit={handleRegister}>
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={labelStyle}>Mot de passe *</label>
                <input type="password" style={inputStyle} value={regPassword} onChange={e => setRegPassword(e.target.value)} placeholder="6 caracteres minimum" required />
              </div>
              <div style={{ marginBottom: 20 }}>
              ...
              <button type="submit" disabled={regLoading} style={{ width: '100%', background: '#1C3829', color: '#fff', border: 'none', borderRadius: 8, padding: '12px', fontWeight: 600, fontSize: 14, cursor: 'pointer', fontFamily: 'Inter, sans-serif', opacity: regLoading ? 0.6 : 1 }}>
                {regLoading ? 'Creation...' : 'Creer mon compte'}
              </button>
            </form>
            <div style={{ textAlign: 'center', marginTop: 16, fontSize: 13, color: '#6b7280' }}>
              Deja un compte ?{' '}
